<div class="fi-topbar-item relative" x-data="{ open: false }" @click.away="open = false">
    <button class="fi-topbar-item-btn" type="button" @click="open = ! open">
        <div class="fi-topbar-item-label flex items-center gap-2">
            <span class="text-sm font-medium">{{ $this->selectedLabel }}</span>
            @if ($this->selectedSiteId)
                @php($selected = $sites->firstWhere('id', $this->selectedSiteId))
                @if ($selected)
                    <x-filament::badge :color="$this->statusColor($selected->status)">
                        {{ str($selected->status)->replace('_', ' ')->title() }}
                    </x-filament::badge>
                @endif
            @endif
        </div>
    </button>

    <div
        x-cloak
        x-show="open"
        x-transition
        class="absolute z-50 mt-2 w-[28rem] rounded-xl border border-gray-200 bg-white p-2 shadow-xl dark:border-gray-800 dark:bg-gray-900"
    >
        <div class="space-y-1">
            <button
                type="button"
                wire:click="selectSite('all')"
                class="flex w-full items-center justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-gray-100 dark:hover:bg-gray-800"
            >
                <span class="font-medium">All sites</span>
                @if (! $this->selectedSiteId)
                    <x-filament::badge color="gray">Selected</x-filament::badge>
                @endif
            </button>

            @forelse ($sites as $site)
                <button
                    type="button"
                    wire:click="selectSite('{{ $site->id }}')"
                    class="flex w-full items-center justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-gray-100 dark:hover:bg-gray-800"
                >
                    <span>
                        <span class="font-medium">{{ $site->display_name }}</span>
                        <span class="text-gray-500"> · {{ $site->apex_domain }}</span>
                    </span>
                    <span class="flex items-center gap-2">
                        <x-filament::badge :color="$this->statusColor($site->status)">
                            {{ str($site->status)->replace('_', ' ')->title() }}
                        </x-filament::badge>
                        @if ((int) $this->selectedSiteId === (int) $site->id)
                            <x-filament::badge color="gray">Selected</x-filament::badge>
                        @endif
                    </span>
                </button>
            @empty
                <div class="rounded-lg px-3 py-2 text-sm text-gray-600 dark:text-gray-300">
                    No sites yet.
                </div>
            @endforelse
        </div>

        <div class="mt-2 border-t border-gray-200 pt-2 dark:border-gray-800">
            <a href="{{ $this->addSiteUrl() }}" class="block rounded-lg px-3 py-2 text-sm font-medium text-primary-600 hover:bg-gray-100 dark:hover:bg-gray-800">
                + Add site
            </a>
        </div>
    </div>
</div>
