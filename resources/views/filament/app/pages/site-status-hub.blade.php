    ...
    >
        ...
        @else
            <x-filament::section
                ...
                description="Follow this setup flow in order. Traffic should not be pointed to CloudFront until deployment is complete."
            ...
            >
                ...
                @if ($this->shouldPollStatus())
                    <p style="margin-top: 0.75rem; opacity: 0.7;">Auto-refreshing status every 3 seconds.</p>
                @endif
            </x-filament::section>
            ...
            <x-filament::section heading="Next Best Action" icon="heroicon-o-forward">
                ...
                @elseif ($this->site->status === \App\Models\Site::STATUS_ACTIVE)
                    ...
                    <div style="display: inline-flex; gap: 0.5rem; flex-wrap: wrap;">
                        <x-filament::button tag="a" :href="\App\Filament\App\Pages\Dashboard::getUrl(['site_id' => $this->site->id])">
                            Open overview
                        </x-filament::button>
                        <x-filament::button color="gray" wire:click="purgeCache">Purge cache</x-filament::button>
                    </div>
                @elseif ($this->site->status === \App\Models\Site::STATUS_FAILED)
            ...
            @if ($this->site->status === \App\Models\Site::STATUS_PENDING_DNS_VALIDATION)
                                ...
                                <div class="fp-dns-row" x-data="{ copied: '' }">
                                    ...
                                    <div class="fp-dns-copy-group">
                                        <x-filament::button
                                            color="gray"
                                            size="sm"
                                            x-on:click="navigator.clipboard.writeText(@js((string) data_get($record, 'name'))); copied = 'Name copied'"
                                        >
                                            Copy name
                                        </x-filament::button>
                                        <x-filament::button
                                            color="gray"
                                            size="sm"
                                            x-on:click="navigator.clipboard.writeText(@js((string) data_get($record, 'value'))); copied = 'Value copied'"
                                        >
                                            Copy value
                                        </x-filament::button>
            ...
            @if ($this->site->status === \App\Models\Site::STATUS_READY_FOR_CUTOVER)
                                ...
                                <div class="fp-dns-row" x-data="{ copied: '' }">
                                    ...
                                    <div class="fp-dns-copy-group">
                                        <x-filament::button
                                            color="gray"
                                            size="sm"
                                            x-on:click="navigator.clipboard.writeText(@js($record['host'])); copied = 'Host copied'"
                                        >
                                            Copy host
                                        </x-filament::button>
                                        <x-filament::button
                                            color="gray"
                                            size="sm"
                                            x-on:click="navigator.clipboard.writeText(@js($record['value'])); copied = 'Target copied'"
                                        >
                                            Copy target
                                        </x-filament::button>
                                        <span x-text="copied"></span>
