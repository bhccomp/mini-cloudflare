        ...
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            @foreach ($items as $item)
                <x-filament::section compact secondary @class([
                    'fp-status-card',
                    'fp-status-card-featured' => $item['label'] === 'Protection',
                    'fp-status-card-attention' => $item['label'] === 'WordPress' && in_array($item['color'], ['warning', 'danger'], true),
                    'fp-status-card-muted' => in_array($item['label'], ['Billing', 'Cache'], true),
