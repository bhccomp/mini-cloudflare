...
class ProvisionWafWebAclJob implements ShouldQueue
{
    ...
    public function handle(EdgeProviderManager $providers): void
    {
        ...
        try {
            ...
            $this->audit($site, 'waf.provision', 'success', $result['message'] ?? 'Edge deployment updated.', $result + ['provider' => $provider->key()]);
        } catch (\Throwable $e) {
            $site->update(['status' => Site::STATUS_FAILED, 'last_error' => $e->getMessage()]);
            $this->audit($site, 'waf.provision', 'failed', $e->getMessage(), ['provider' => $provider->key()]);
            throw $e;
        }
    }
