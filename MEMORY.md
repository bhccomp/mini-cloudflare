...
## Blog Webhooks + OpenAI Covers + Outrank Cleanup (Latest)
- FirePhage now has a shared blog import pipeline for external SEO/article tools.
- Current core service:
  - `app/Services/Marketing/BlogImportService.php`
- Current behavior of the shared import pipeline:
  - creates or updates posts by slug
  - defaults author to:
    - `Nikola Jocic`
  - defaults category to:
    - `wordpress-security`
  - normalizes canonical URL to FirePhage blog routes if the upstream `public_url` is off-domain
  - sanitizes imported markdown before saving
  - strips imported image blocks from article bodies
  - can optionally prepend a final cover image block at the top of the article body
- Current imported-attribution cleanup rules:
  - removes BabyLoveGrowth footer tails like:
    - `Article generated by BabyLoveGrowth`
  - removes Outrank footer tails like:
    - `Enhanced by the Outrank app`
    - `Enhanced by Outrank`
    - linked/italic markdown variants pointing to `https://outrank.so`
- BabyLoveGrowth webhook:
  - route:
    - `/webhooks/babylovegrowth/blog-post`
  - controller:
    - `app/Http/Controllers/Webhooks/BabyLoveGrowthBlogWebhookController.php`
  - config key:
    - `BABYLOVEGROWTH_BLOG_WEBHOOK_TOKEN`
  - current behavior:
    - uses the shared import service
    - keeps OpenAI cover generation enabled by default
- Outrank webhook:
  - route:
    - `/webhooks/outrank/blog-post`
  - controller:
    - `app/Http/Controllers/Webhooks/OutrankBlogWebhookController.php`
  - config key:
    - `OUTRANK_BLOG_WEBHOOK_TOKEN`
  - current behavior:
    - accepts batched `data.articles[]` payloads
    - uses the shared import service
    - explicitly skips OpenAI cover generation so webhook tests do not time out
    - uses incoming `image_url` as the cover/OG image if present
- OpenAI blog cover generation:
  - service:
    - `app/Services/Marketing/OpenAiBlogCoverImageService.php`
  - config:
    - `OPENAI_API_KEY`
  ...
  - current behavior:
    ...
    - saves local FirePhage-hosted cover images under:
      - `public/images/blog-covers/`
    - crops/resizes to a wide website hero format
  - command:
    - `php artisan blog:generate-cover {slug...}`
    - implementation:
      - `app/Console/Commands/GenerateBlogCoverCommand.php`
- Current important operational split:
  - BabyLoveGrowth imports:
    - `generate_cover = true`
  - Outrank imports:
    - `generate_cover = false`
- Existing imported post cleanup done:
  - the three OpenAI-regenerated posts now reference only FirePhage-hosted cover images
  - `how-do-i-block-ip-address` had its Outrank attribution removed from stored markdown and from rendered output after cache clear
